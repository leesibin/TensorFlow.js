 ...
 * 오차 행렬을 그립니다.
 *
 * @param {tf.Tensor} confusionMat 2D tf.Tensor 객체인 오차 행렬.
 *   행 `r`과 열 `c`에 있는 값은 실제 클래스 `r`을 클래스 `c`로 예측한 샘플의 개수입니다.
 */
export function drawConfusionMatrix(confusionMat) {
  ...
  const normalizedConfusion = confusionMat
    .div(confusionMat.sum(-1).expandDims(0))
    .dataSync();
  for (let i = 0; i < n; ++i) {
    for (let j = 0; j < n; ++j) {
